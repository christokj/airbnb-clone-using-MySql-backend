...
const Place = {
    // Create a new place
    createPlace: async (ownerId, title, address, photos, description, perks, extraInfo, checkIn, checkOut, maxGuests, price) => {
        ...
        return new Promise((resolve, reject) => {
            db.query(sql, [ownerId, title, address, JSON.stringify(photos), description, JSON.stringify(perks), extraInfo, checkIn, checkOut, maxGuests, price], (err, result) => {
                if (err) {
                    reject(err);
                } else {
    ...
    updatePlace: async (id, title, address, photos, description, perks, extraInfo, checkIn, checkOut, maxGuests, price) => {
        const sql = `UPDATE places SET title = ?, address = ?, photos = ?, description = ?, perks = ?, extraInfo = ?, checkIn = ?, checkOut = ?, maxGuests = ?, price = ? WHERE id = ?`;
        return new Promise((resolve, reject) => {
            db.query(sql, [title, address, JSON.stringify(photos), description, JSON.stringify(perks), extraInfo, checkIn, checkOut, maxGuests, price, id], (err, result) => {
                if (err) {
                    reject(err);
                } else {
