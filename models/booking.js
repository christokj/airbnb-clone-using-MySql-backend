...
const Booking = {
    ...
    createBooking: async (placeId, userId, checkIn, checkOut, name, phone, price) => {
        const sql = `INSERT INTO bookings (place, user, checkIn, checkOut, name, phone, price)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`;
        return new Promise((resolve, reject) => {
            db.query(sql, [placeId, userId, checkIn, checkOut, name, phone, price], (err, result) => {
                if (err) {
                    reject(err);
                } else {
    ...
    updateBooking: async (id, placeId, userId, checkIn, checkOut, name, phone, price) => {
        const sql = `UPDATE bookings SET place = ?, user = ?, checkIn = ?, checkOut = ?, name = ?, phone = ?, price = ? WHERE id = ?`;
        return new Promise((resolve, reject) => {
            db.query(sql, [placeId, userId, checkIn, checkOut, name, phone, price, id], (err, result) => {
                if (err) {
                    reject(err);
                } else {
